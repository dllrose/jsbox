  ...
  $http.request({
    ...
    handler: function(resp) {
      ...
      match.map(function(i) {
        var link = /自定义_连接_正则/.exec(i)[1];
        var image = /自定义_图片_正则/.exec(i)[1];
        var title = /自定义_标题_正则/.exec(i)[1];
...
function getDetail(url) {      //弹出详情 这里调整连接
...
function main() {
  ...
  $http.request({
    ...
    url: url,
    handler: function(resp) {
      var match = /自定义_首页正则/g.exec(resp.data)
      if (match) {
        //$ui.toast("载入成功", 1)
        page = 0
        //$ui.action(match)
